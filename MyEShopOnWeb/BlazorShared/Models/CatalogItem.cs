...
namespace BlazorShared.Models
{
    public class CatalogItem
    {
        ...
        [RegularExpression(@"^\d+(\.\d{0,2})*$", ErrorMessage = "The field Price must be a positive number with maximun two decimals.")]
        [Range(0.01, 1000)]
        ...
        public static string IsValidImage(string pictureName, string pictureBase64)
        {
            ...
            {
                return "Maximun length is 512KB";
            }
        ...
        public static async Task<string> DataToBase64(IFileListEntry fileItem)
        {
            using (var reader = new StreamReader(fileItem.Data))
            {
                using (var memStream = new MemoryStream())
                {
                    ...
                    var encodeBase64 = Convert.ToBase64String(fileData);
                    return encodeBase64;
                }
            }
        }
